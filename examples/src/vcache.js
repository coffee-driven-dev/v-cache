...
const successHandlerFn = function (value, vnode, restoreTarget) {
  ...
  if (isObject) {
    vnode.componentInstance[restoreTarget] = Object.assign({}, vnode.componentInstance[restoreTarget], value);
  } else {
...
export default {
  ...
  bind: function (el, binding, vnode) {
    ...
    // Restore Cache
    const handleRestoreCache = eventData => localforage.getItem(cacheKey).then(value => {
      if (!value) {
        return value;
      }

      const isCustomSuccessHandler = eventData !== null && typeof eventData === "function";
      const isCustomRestoreTarget = eventData !== null && typeof eventData === "function" && eventData.length > 0;

      let customRestoreTarget = isCustomRestoreTarget ? eventData : restoreTarget;
      isCustomSuccessHandler ? eventData(value, vnode, customRestoreTarget) : successHandler(value, vnode, customRestoreTarget);

      return value;
    }).then(restoreCallback).catch(errorHandler);

    vnode.componentInstance.$on("restore-cache", handleRestoreCache);

    // Clear Cache
    const handleClearCache = () => setTimeout(() => localforage.removeItem(cacheKey), 200);
